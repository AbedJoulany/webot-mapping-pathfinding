...
class RobotController:
    ...
    def _initialize_devices(self):
        ...
        self.map_size = (10, 10)  # Adjust size as per your Webots world
        self.explored_map = np.full(self.map_size, -1)  # Initialize explored map
    ...
    def world2map(self, gps_position, world_size, floor_size):
        ...
        map_x = math.ceil(robot_pose[0]) if robot_pose[0] >= 0 else math.floor(robot_pose[0])
        map_y = math.ceil(robot_pose[1]) if robot_pose[1] >= 0 else math.floor(robot_pose[1])
        print(f'x={map_x}, y = {map_y}')
        map_x = int((robot_pose[0] + world_size[0] // 2) // floor_size)
        map_y = int((robot_pose[1] + world_size[1] // 2) // floor_size)
    ...
    def update_map(self, z_points, current_time, previous_time):
        ...
        for point in z_points:
            map_x, map_y = self.world2map(point, (8,8), 1)
            #self.explored_map[map_x, map_y] = 1  # Mark this cell as explored
        print([map_x, map_y])
        return 0
